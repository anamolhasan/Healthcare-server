...
const getStatusCodeFromPrismaError = (errorCode: string) : number => {
    ...
    // P6003: Prisma Accelarate Plan limit exceed = 402 Payment Required
    if(errorCode === 'P6003'){
        return status.PAYMENT_REQUIRED
    }

    // P1008, P2004, 6004 : Timeout errors = 504 Gateway Timeout
    if(['P1008', 'P2004', '6004'].includes(errorCode)){
    ...
    // P6009 Response size limit exceeded = 413 Payload To Large
    if(errorCode === 'P6009'){
        return 413
    }
...
export const handlePrismaClientValidationError = (error: Prisma.PrismaClientValidationError) : TErrorResponse => {
    ...
    // Remove the "Invalid `Prisma.user.create()` " part from the message for better readability
      cleanMessage = cleanMessage.replace(/Invalid `.*?` invocation:?\s*/i, "")
