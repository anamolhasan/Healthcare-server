...
const givePrescription = async (user:IRequestUser, payload:ICreatePrescriptionPayload) => {
    ...
    if(appointmentData.doctorId !== doctorData.id){
        throw new AppError(status.BAD_REQUEST, 'You can only giv prescription for your own appointments')
    }
        ...
        try {
            ...
            await sendEmail({
                ...
                templateData:{
                    ...
                    prescriptionId: payload.instructions,
...
const updatePrescription = async (user:IRequestUser, prescriptionId :string, payload:any) => {
    ...
    // Step 1: Generate new PDF with update data
    const pdfBuffer = await generatePrescriptionPDF({
        ...
        createAt:prescriptionData.createdAt,
    })
    ...
    // Step 3: Delete old PDF to cloudinary
    ...
    try {
        await sendEmail({
            ...
            templateData:{
                ...
                issueDate: new Date(result.createdAt).toLocaleDateString(),
