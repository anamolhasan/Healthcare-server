...
export const updateDoctorZodSchema = z.object({
    doctor:z.object({
        name:z.string('Name must be string').min(5, 'Name must be at least 5 characters').max(30, 'Name must be at most 30 characters'),
        profilePhoto: z.url('Profile photo must be a valid URL').optional(),
        contactNumber: z.string('Contact number must be a valid URL').min(11, 'Contact number must be at least 11 characters').max(14, 'Contact number must be at most 15 characters').optional(),
        address: z.string('Address must be string').min(10, 'Address must be at least 10 characters').min(100, 'address must be at most 100 characters').optional(),
        ...
        qualification: z.string('Qualification must be string').min(2, 'Qualification must be aty least 2 characters').max(50, 'Qualification must be at most 50 characters').optional(),
    ...
    specialties:z.array(z.object({
        specialtyId: z.uuid('Specialty ID must be valid UUID'),
