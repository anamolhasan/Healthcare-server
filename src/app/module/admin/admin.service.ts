...
const changeUserStatus = async (user: IRequestUser, payload:IChangeUserStatusPayload) => {
    // 1. Super admin can change the status of any user(admin, doctor, patient). Except himself. He cannot chang his own status.
    ...
    if(isAdminExists.user.role === Role.ADMIN && userToChangeStatus.role === Role.SUPER_ADMIN){
        throw new AppError(status.BAD_REQUEST, 'You can change status of super admin. Only super admin can change the status of another super admin');
    }
    ...
    if(userStatus === UserStatus.DELETED){
        throw new AppError(status.BAD_REQUEST, 'You cannot set user status to deleted. To delete a user, you have  to user role specific delete api For example, to delete an doctor user, you have to user delete doctor api which will set the user status to deleted and also set isDeleted to true and also delete the user session and account')
    }
...
const changeUserRole = async (user:IRequestUser, payload : IChangeUserRolePayload) => {
    // 1. Super admin can change the role of only other super admin and admin user. He change his own role.
    ...
    if(selfRoleChange){
        throw new AppError(status.BAD_REQUEST, 'You cannot your own role');
    }

    if(userToChangeRole.role === Role.DOCTOR || userToChangeRole.role === Role.PATIENT){
        throw new AppError(status.BAD_REQUEST, 'You cannot change the role of doctor or patient user. If you want to change the role of doctor of patient user, you have to delete the user and recreate with new role')
    }
