...
export const createDoctorZodSchema = z.object({
    ...
    doctor: z.object({
        ...
        address: z.string('Address is required ').min(10, 'Address must be ata least 10 characters').max(100, 'Address must be ata most 100 characters').optional(),
        ...
        currentWorkingPlace: z.string('Current working place is required').min(2, 'Current working place must be at least 2 characters').max(50, 'Current working place must be st most 50 characters'),

        designation: z.string('Designation is required').min(2, 'Designation must be st least 2 characters').max(50, 'Designation must be at most 50 characters'),
    }),
    specialties: z.array(z.uuid(), 'Specialties must be an array of strings').min(1, 'At least one specialty is required')
})

export const createdAdminZodSchema = z.object({
    password: z.string('Password is required').min(6, 'Password must be at least 6 characters').max(20, 'Password must be at most 20 characters'),
    admin: z.object({
        name:z.string('Name is required and must be string').min(5, 'Name must be at least 5 characters').max(30, 'Name must be at most 30 characters'),
        email: z.email('Invalid email address'),
